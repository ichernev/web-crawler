...
public class KeywordController {
    ...
    public String addNewKeyword(String name) {
        ...
        List<Keyword> keywords = currentUser.getKeywords();
    ...
    @PostMapping("/user/deleteKeyword/{id}")public String deleteUserKeyword(@PathVariable("id") Integer id) {
        Optional<Keyword> keywordToDelete = keywordRepository.findById(id);
        if(keywordToDelete.isEmpty()) {        return "redirect:/user/keywords?deleted=failure";
        }
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        MyUser currentUser = userRepository.findByEmail(authentication.getName()).get();
        List<Keyword> userKeywords = currentUser.getKeywords();
        int indexToDelete = userKeywords.indexOf(keywordToDelete.get());
        if (indexToDelete == -1) { // if the keyword is not in user's keywords list        return "redirect:/user/keywords?deleted=failure";
        }
